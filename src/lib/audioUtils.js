...
function drawSpectrum(audioData) {
	...
	for (var i = 0; i < audioData.length; i++) {
		...
		canvas.context.fillStyle = "rgb(" + (255 - barHeight) + "," + (255 - barHeight) + "," + (255 - barHeight) + ")";
...
function getPitch(buffer, sampleRate, threshold = 0.2) {
	...
	var note = noteNames[audioUtils.noteFromPitch(autoCorrelateValue) % noteNames.length] ?? null;
